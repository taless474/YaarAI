...
# Extract beyt-level semantic annotations (v1.0)
...
OUT_PATH = Path("data/annotations/beyt_annotations_v1.jsonl")
...
def load_done_keys() -> set[tuple]:
    """Return (poem_id, beyt_id) already processed."""
...
def validate_annotation(obj: dict):
    ...
    assert "beyt_hint" in obj
    assert "affect" in obj
    assert isinstance(obj["beyt_hint"], str)
...
def call_gpt_until_valid(prompt: str, meta: dict, max_attempts: int = 2) -> dict:
    ...
    print(
        ...
        f"beyt_id={meta['beyt_id']} affect=[]"
    )

    return {
        "beyt_hint": annotation.get("beyt_hint", "").strip(),
        "affect": [],
    }
...
def main():
    ...
    with RAW_PATH.open("r", encoding="utf-8") as fin, \
        ...
        for line in fin:
            row = json.loads(line)
            key = (row["poem_id"], row["beyt_id"])

            if key in done:
                continue

            prompt = BEYT_PROMPT.format(
                affect_list="، ".join(AFFECT_VOCAB),
                ghazal_axis=axis_map[row["poem_id"]],
                beyt_text=row["text"],
                beyt_prose=row["insight"]["beyt_summary"],
            )
            ...
            record = {
                "poem_id": row["poem_id"],
                "beyt_id": row["beyt_id"],
                "beyt_hint": annotation["beyt_hint"],
            ...
            print(f"[OK] poem_id={row['poem_id']} beyt_id={row['beyt_id']}")
